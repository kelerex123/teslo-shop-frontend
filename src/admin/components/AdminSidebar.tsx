...
export const AdminSidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggle }) => {
  ...
  const isActiveRoute = (to: string) => {
    // TODO: ajustarlo cuando estemos en pantalla de producto
          ...
          {menuItems.map((item, index) => {
            ...
            return (
              <li key={index}>
                <Link
                  to={item.to || '/admin'}
                  className={`flex items-center space-x-3 px-3 py-2 rounded-lg transition-all duration-200 group ${isActiveRoute(item.to || '/xd')
